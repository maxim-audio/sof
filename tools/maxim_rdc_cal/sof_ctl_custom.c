...
#ifdef DSM_CUSTOMIZATION
static int read_setup(struct ctl_data *ctl_data, struct dsm_param_t *param)
#else
...
#endif
{
	...
	#ifdef DSM_CUSTOMIZATION
	if (ctl_data->binary) {
		/* Update ctl_data->buffer for writing */

		memcpy(&ctl_data->buffer[BUFFER_ABI_OFFSET + abi_size],
			param->buf, param->size * param->max_ch * SZ_PER_PARAM * sizeof(int));
		n = param->size * param->max_ch * SZ_PER_PARAM;
	}
	#else
	if (ctl_data->binary) {
...
#ifdef DSM_CUSTOMIZATION
static int ctl_setup(struct ctl_data *ctl_data, struct dsm_param_t *param)
#else
...
#endif
{
	...
	if (ctl_data->binary && ctl_data->set) {
		...
		#ifdef DSM_CUSTOMIZATION
		ctrl_size = param->size * param->max_ch * SZ_PER_PARAM * sizeof(int);
		#else
...
#ifdef DSM_CUSTOMIZATION
static int ctl_set_get(struct ctl_data *ctl_data, struct dsm_param_t *param)
#else
...
#endif
{
	...
	if (ctl_data->set) {
		...
		if (!param->init)
			param->size = ctl_data->buffer[DSM_SZ_OFFSET];
		else
			memcpy(param->buf, &ctl_data->buffer[DSM_PARAM_OFFSET],
				param->size * SZ_PER_PARAM * param->max_ch * sizeof(int));
		#endif
	}
	return ret;
}
...
#ifdef DSM_CUSTOMIZATION
int dsm_access_param(int is_write, struct dsm_param_t *param)
{
	...
	memset(ctl_data, 0, sizeof(struct ctl_data));

	/* hardcoded for the DSM audio component */
	ctl_data->dev = "hw:0";
	ctl_data->type = 1;
	...
	if (is_write) {
		...
		ctl_data->cname = "numid=152";
		ctl_data->set = true;
	} else {
		ctl_data->cname = "numid=153";
		ctl_data->set = false;
	}
	/* set up ctl_elem, allocate buffers */
	ret = ctl_setup(ctl_data, param);
	if (ret < 0) {
		fprintf(stderr, "Error: ctl_data setup failed, ret:%d\n", ret);
		goto in_fd_close;
	}

	/* set/get the tlv bytes kcontrol */
	ret = ctl_set_get(ctl_data, param);
	if (ret < 0) {
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt(argc, argv, "hD:c:s:n:o:t:g:br")) != -1) {
		...
		case 'c':
			...
			break;
		case 'n':
			sfprintf(stdout, nname, "numid=%d", atoi(optarg));
