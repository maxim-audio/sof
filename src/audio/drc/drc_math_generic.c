...
#define q_mult(a, b, qa, qb, qy) ((int32_t)Q_MULTSR_32X32((int64_t)a, b, qa, qb, qy))
	...
	/* frexpf implementation */
	while (x_fixed < HALF_Q_V || x_fixed >= ONE_Q_V) {
		if (x_fixed < HALF_Q_V) {
			x_fixed = Q_SHIFT_LEFT(x_fixed, q_v, q_v + 1);
			e -= 1 << 1; /* Q_CONVERT_FLOAT(1, 1) */
		} else {
			x_fixed = Q_SHIFT_RND(x_fixed, q_v, q_v - 1);
			e += 1 << 1; /* Q_CONVERT_FLOAT(1, 1) */
		}
	}
